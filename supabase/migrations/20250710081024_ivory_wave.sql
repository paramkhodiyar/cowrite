/*
  # Fix Firebase UID Schema Migration

  This migration converts user ID columns from UUID to TEXT to support Firebase UIDs.
  
  1. Schema Changes
    - Convert profiles.id from uuid to text
    - Convert all foreign key columns referencing profiles.id to text
    - Handle the auth.users foreign key constraint properly
  
  2. Constraint Management
    - Drop and recreate all foreign key constraints
    - Handle the profiles.id -> auth.users.id constraint specially
